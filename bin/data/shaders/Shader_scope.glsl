...
vec2 getPoint(float x) {
    ...
    float pointX = texture(iChannel0,vec2(left, 1.0)).x;
    float pointY = texture(iChannel0,vec2(right, 1.0)).x;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float posOffset = texture(iChannel1, (fragCoord.xy / 256.0)).x;
    float timeOffset = iTime * 343.42;
    ...
    vec3 color = vec3(1.0, 0.13, 0.05) * acc;
